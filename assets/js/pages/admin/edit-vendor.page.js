parasails.registerPage('edit-vendor', {
  //  ╦╔╗╔╦╔╦╗╦╔═╗╦    ╔═╗╔╦╗╔═╗╔╦╗╔═╗
  //  ║║║║║ ║ ║╠═╣║    ╚═╗ ║ ╠═╣ ║ ║╣
  //  ╩╝╚╝╩ ╩ ╩╩ ╩╩═╝  ╚═╝ ╩ ╩ ╩ ╩ ╚═╝
  data: {
  ...
  methods: {
    changeVendorImageInput: function(files) {
      if (files.length !== 1 && !this.updatedVendor.photo) {
      ...
      if (!selectedFile && this.updatedVendor.photo) {
        return;
      }

      this.imageName = selectedFile.name;
      this.updatedVendor.image = selectedFile;
