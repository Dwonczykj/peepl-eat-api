      ...
      return _regeneratorRuntime().wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              that = this; //Subscribe to messages about order

              io.socket.get('/api/v1/orders/subscribe-to-order/' + that.order.id); // Order marked as paid

              io.socket.on('paid', function (data) {
                that.order.paidDateTime = data.paidDateTime;

                _paq.push(['trackEvent', 'eCommerce', 'Completed order', data.orderId, _this.cartTotal + _this.deliveryTotal]);
              });
